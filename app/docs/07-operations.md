...
1. Yandex Cloud отправляет ВМ сигнал на остановку через metadata service.
   Уведомление приходит **за 30 секунд** до принудительной остановки.

2. Через 30 секунд ВМ принудительно выключается. При этом:
   - kubelet **не успевает корректно завершить поды** (graceful shutdown).
   - Поды не получают сигнал SIGTERM, если он не был отправлен
     приложением самостоятельно.
   - На узле остаются «зависшие» поды в статусах `Completed`,
     `Error`, `ContainerStatusUnknown`.

3. Kubernetes обнаруживает, что узел не отвечает, и помечает его
   как `NotReady`. Это происходит не мгновенно, а через
   `node-monitor-grace-period` (по умолчанию 40 секунд).

4. Поды на этом узле начинают эвакуироваться только после
   `default-not-ready-tolerationSeconds` (по умолчанию **300 секунд**).
   То есть реальная эвакуация запускается через **5–6 минут**
   после остановки ВМ. За это время пользователи могут видеть
   ошибки в приложении.

5. ВМ **не запускается автоматически**. Она остаётся выключенной,
   пока её не запустят вручную (или через Cloud Function по расписанию).
   В отличие от Instance Groups, где auto-healing перезапускает
   узел автоматически.

6. После ручного запуска ВМ kubelet регистрируется в кластере,
   узел снова становится `Ready`. Поды, которые были эвакуированы
   на другие узлы, останутся там (Kubernetes не возвращает их
   обратно). Если ёмкости других узлов не хватило, часть подов
   может оставаться в `Pending`.

### Как смягчить последствия прерывания

1. **Установить DaemonSet `yc-mk8s-node-shutdown-script`.**
   Он выполняет `kubectl drain --ignore-daemonsets` на узле
   до его выключения. Поды корректно эвакуируются, не остаётся
   «мусорных» подов. Это официальный пример от Yandex Cloud[reference:3].

2. **Иметь больше реплик приложения, чем узлов.**
   При 2 репликах и 2 узлах — если один узел выключается,
   вторая реплика остаётся. При 3 репликах и 2 узлах —
   переживёт падение одного узла без потери ёмкости.

3. **Настроить `PodDisruptionBudget` (PDB).**
   PDB не поможет при прерывании (это не voluntary disruption),
   но защитит от плановых drain-операций.

4. **Автоматический перезапуск ВМ через Cloud Function.**
   По аналогии с официальным руководством Yandex Cloud[reference:4].
   Функция раз в 5 минут проверяет состояние ВМ и запускает
   выключенные.

5. **Рассмотреть Instance Groups вместо отдельных ВМ.**
   Они умеют auto-healing и перезапускают прерываемые ВМ
   автоматически[reference:5]. Но с external nodes Managed K8s
   это не применимо — там узлы подключаются через NodeGroup CRD.

### Итог

При использовании прерываемых ВМ **автоматического восстановления
узлов не происходит**. Это осознанный компромисс за скидку 50–70%.
Для production такие сценарии требуют дополнительных механизмов
(Instance Groups, Cloud Functions, DaemonSet для drain).
